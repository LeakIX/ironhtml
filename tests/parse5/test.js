/**
 * Integration tests using parse5 to validate HTML output from html-builder.
 *
...
try {
  ...
  console.log(
    "  ⚠ Skipping Rust integration tests (example not available yet)\n"
  );
  samples = null;
}
...
test("parse5 handles unicode", () => {
  ...
  const p = findElement(doc, "p");
  const text = getTextContent(p);
  assert.ok(text.includes("世界"), "unicode should be preserved");
  assert.ok(text.includes("🌍"), "emoji should be preserved");
});
