...
use ironhtml_elements::{Button, Div, Li, Ul, A};
...
///
/// ```rust
/// use ironhtml_bootstrap::list_group::{list_group_links, ListGroupLink};
///
/// let items = vec![
...
fn list_group_link_item(item: &ListGroupLink) -> Element<A> {
    ...
    if let Some(color) = item.color {
        use core::fmt::Write;
        let _ = write!(class, " list-group-item-{}", color.as_str());
    }
