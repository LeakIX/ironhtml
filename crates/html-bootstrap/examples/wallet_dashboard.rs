...
impl Network {
    ...
    fn badge_color(&self) -> Color {
        match self {
            Network::Mainnet => Color::Success,
            Network::Testnet => Color::Warning,
        }
    }
}
...
fn balance_card(balance: &WalletBalance, symbol: &str, has_pending: bool) -> Element<Div> {
    cards::card(|body| {
        let body = body
            ...
            .child::<H2, _>(|h| {
                let amount = format!("{} {:.8}", symbol, balance.total);
...
fn transaction_row(tx: &Transaction, symbol: &str) -> Element<Tr> {
    ...
    let amount_text = format!("{}{:.8} {}", amount_prefix, tx.amount.abs(), symbol);
...
fn main() {
    ...
    println!("{}", generate_wallet_page(&mainnet_wallet).render());

    println!("\n\n=== TESTNET WALLET (syncing, empty) ===\n");
    println!("{}", generate_wallet_page(&testnet_wallet).render());
